...
public:
	...
	T Max();
	void PrintHeap(int iLength);
	void Sort(T tArray[], int iLenght);
...
template<typename T> void MaxHeap<T>::PrintHeap(int iLength) {
	for(int i = 1; i <= iLength; ++i)
		cout << tData[i] << " ";
	cout << endl;
}
...
template<typename T> void MaxHeap<T>::Sort(T tArray[], int iLength) {
